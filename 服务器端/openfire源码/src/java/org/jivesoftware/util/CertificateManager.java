 ...
 * Generated certificates conform to the XMPP spec where domains are kept in the
...
public class CertificateManager {
	...
	private static final Logger Log = LoggerFactory
			.getLogger(CertificateManager.class);
	...
	/**
	 * Creates a new X509 certificate using the DSA algorithm. The new
	 * certificate together with its private key are stored in the specified key
	 * store. However, the key store is not saved to the disk. This means that
	 * it is up to the "caller" to save the key store to disk after new
	 * certificates have been added to the store.
	 * 
	 * @param ksKeys
	 *            key store where the new certificate and private key are going
	 *            to be stored.
	 * @param keyPassword
	 *            password of the keystore.
	 * @param alias
	 *            name to use when storing the certificate in the key store.
	 * @param issuerDN
	 *            Issuer string e.g "O=Grid,OU=OGSA,CN=ACME"
	 * @param subjectDN
	 *            Subject string e.g "O=Grid,OU=OGSA,CN=John Doe"
	 * @param domain
	 *            domain of the server to store in the subject alternative name
	 *            extension.
	...
	public static X509Certificate createDSACert(KeyStore ksKeys,
			String keyPassword, String alias, String issuerDN,
			String subjectDN, String domain) throws GeneralSecurityException,
			IOException {
		// Generate public and private keys
		KeyPair keyPair = generateKeyPair("DSA",
				JiveGlobals.getIntProperty("cert.dsa.keysize", 1024));
		// Create X509 certificate with keys and specified domain
		X509Certificate cert = createX509V3Certificate(keyPair, 60, issuerDN,
				subjectDN, domain, "SHA1withDSA");
		// Store new certificate and private key in the keystore
		ksKeys.setKeyEntry(alias, keyPair.getPrivate(),
	...
	/**
	 * Creates a new X509 certificate using the RSA algorithm. The new
	 * certificate together with its private key are stored in the specified key
	 * store. However, the key store is not saved to the disk. This means that
	 * it is up to the "caller" to save the key store to disk after new
	 * certificates have been added to the store.
	...
	public static X509Certificate createRSACert(KeyStore ksKeys,
		...
		KeyPair keyPair = generateKeyPair("RSA",
				JiveGlobals.getIntProperty("cert.rsa.keysize", 2048));
		// Create X509 certificate with keys and specified domain
		X509Certificate cert = createX509V3Certificate(keyPair, 60, issuerDN,
				subjectDN, domain, "SHA1WITHRSAENCRYPTION");
		// Store new certificate and private key in the keystore
		ksKeys.setKeyEntry(alias, keyPair.getPrivate(),
				keyPassword.toCharArray(), new X509Certificate[] { cert });
		// Notify listeners that a new certificate has been created
	 ...
	 * 
	 * @param ksKeys
	 *            key store where the certificate is stored.
	 * @param alias
	...
	/**
	 * Returns the identities of the remote server as defined in the specified
	 * certificate. The identities are defined in the subjectDN of the
	 * certificate and it can also be defined in the subjectAltName extensions
	 * of type "xmpp". When the extension is being used then the identities
	 * defined in the extension are going to be returned. Otherwise, the value
	 * stored in the subjectDN is returned.
	 * 
	 * @param x509Certificate
	 *            the certificate the holds the identities of the remote server.
	 * @return the identities of the remote server as defined in the specified
	 *         certificate.
	 */
	...
	private static List<String> getSubjectAlternativeNames(
		...
		try {
			...
			for (List item : altNames) {
				...
				if (type == 0) {
					...
					try {
						...
						ASN1InputStream decoder = new ASN1InputStream(
								(byte[]) item.get(1));
						ASN1Sequence otherNameSeq = (ASN1Sequence) decoder
								.readObject();

						// Check the object identifier
						DERObjectIdentifier objectId = (DERObjectIdentifier) otherNameSeq
								.getObjectAt(0);
						Log.debug("Parsing otherName for subject alternative names: "
								+ objectId.toString());

						if (!OTHERNAME_XMPP_OID.equals(objectId.getId())) {
							// Not a XMPP otherName
							Log.debug("Ignoring non-XMPP otherName, "
									+ objectId.getId());
							continue;
						}
						...
						try {
							...
							ASN1Encodable o = (ASN1Encodable) otherNameSeq
									.getObjectAt(1);
							if (o instanceof DERTaggedObject) {
								ASN1TaggedObject ato = DERTaggedObject
										.getInstance(o);
								Log.debug("... processing DERTaggedObject: "
										+ ato.toString());
								// TODO: there's bound to be a better way...
								identity = ato
										.toString()
										.substring(
												ato.toString().lastIndexOf(']') + 1)
										.trim();
							} else {
								DERUTF8String derStr = DERUTF8String
										.getInstance(o);
								identity = derStr.getString();
							}
							if (identity != null && identity.length() > 0) {
								// Add the decoded server name to the list of
								// identities
								identities.add(identity);
							}
						} catch (IllegalArgumentException ex) {
							// OF-517: othername formats are extensible. If we
							// don't recognize the format, skip it.
							Log.debug(
				...
				// Other types are not applicable for XMPP, so silently ignore
				// them
			}
		} catch (CertificateParsingException e) {
			Log.error("Error parsing SubjectAltName in certificate: "
	...
	/**
	 * Returns true if an RSA certificate was found in the specified keystore
	 * for the specified domain.
	 * 
	 * @param ksKeys
	 *            the keystore that contains the certificates.
	 * @param domain
	 *            domain of the server signed by the certificate.
	 * @return true if an RSA certificate was found in the specified keystore
	 *         for the specified domain.
	 * @throws KeyStoreException
	 */
	public static boolean isRSACertificate(KeyStore ksKeys, String domain)
			throws KeyStoreException {
		return isCertificate(ksKeys, domain, "RSA");
	}

	/**
	 * Returns true if an DSA certificate was found in the specified keystore
	 * for the specified domain.
	 * 
	 * @param ksKeys
	 *            the keystore that contains the certificates.
	 * @param domain
	 *            domain of the server signed by the certificate.
	 * @return true if an DSA certificate was found in the specified keystore
	 *         for the specified domain.
	 * @throws KeyStoreException
	 */
	public static boolean isDSACertificate(KeyStore ksKeys, String domain)
			throws KeyStoreException {
		return isCertificate(ksKeys, domain, "DSA");
	}

	/**
	 * Returns true if the specified certificate is using the DSA algorithm. The
	 * DSA algorithm is not good for encryption but only for authentication. On
	 * the other hand, the RSA algorithm is good for encryption and
	 * authentication.
	 * 
	 * @param certificate
	...
	/**
	 * Returns true if a certificate with the specified configuration was found
	 * in the key store.
	 * 
	 * @param ksKeys
	 *            the keystore to use for searching the certificate.
	 * @param domain
	 *            the domain present in the subjectAltName or "*" if anything is
	 *            accepted.
	 * @param algorithm
	 *            the DSA or RSA algorithm used by the certificate.
	 * @return true if a certificate with the specifed configuration was found
	 *         in the key store.
	 * @throws KeyStoreException
	 */
	private static boolean isCertificate(KeyStore ksKeys, String domain,
			String algorithm) throws KeyStoreException {
		boolean result = false;
		for (Enumeration<String> aliases = ksKeys.aliases(); aliases
				.hasMoreElements();) {
			X509Certificate certificate = (X509Certificate) ksKeys
		...
		Log.debug(
				"Check for certificate for '{}' using algorithm {} returned: {}",
	...
	/**
	 * Returns true if the specified certificate is a self-signed certificate.
	 * If the certificate was not found in the store then a KeyStoreException is
	 * returned.
	 * 
	 * @param keyStore
	 *            key store that holds the certificate to verify.
	 ...
	 */
	public static boolean isSelfSignedCertificate(KeyStore keyStore,
			X509Certificate certificate) throws KeyStoreException {
		String alias = keyStore.getCertificateAlias(certificate);
		if (alias == null) {
			throw new KeyStoreException("Certificate not found in store: "
					+ certificate);
		}
	...
	/**
	 * Returns true if the specified certificate is ready to be signed by a
	 * Certificate Authority. Self-signed certificates need to get their issuer
	 * information entered to be able to generate a Certificate Signing Request
	 * (CSR).
	 * 
	 * @param keyStore
	 *            key store that holds the certificate to verify.
	...
	public static boolean isSigningRequestPending(KeyStore keyStore,
		...
		Matcher matcher = valuesPattern.matcher(certificate.getIssuerDN()
				.toString());
		return matcher.find() && matcher.find();
	}

	/**
	 * Creates and returns the content of a new singing request for the
	 * specified certificate. Signing requests are required by Certificate
	 * Authorities as part of their signing process. The signing request
	 * contains information about the certificate issuer, subject DN, subject
	 * alternative names and public key. Private keys are not included. After
	 * the Certificate Authority verified and signed the certificate a new
	 * certificate is going to be returned. Use
	 * {@link #installReply(java.security.KeyStore, java.security.KeyStore, String, String, java.io.InputStream, boolean, boolean)}
	 * to import the CA reply.
	 * 
	 * @param cert
	 *            the certificate to create a signing request.
	 * @param privKey
	 *            the private key of the certificate.
	 * @return the content of a new singing request for the specified
	 *         certificate.
	 * @throws Exception
	 */
	public static String createSigningRequest(X509Certificate cert,
		...
		String signatureAlgorithm = "DSA".equals(pubKey.getAlgorithm()) ? "SHA1withDSA"
				: "SHA1WITHRSAENCRYPTION";

		PKCS10CertificationRequest csr = new PKCS10CertificationRequest(
				signatureAlgorithm, xname, pubKey, null, privKey);

		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		DEROutputStream deros = new DEROutputStream(baos);
		deros.writeObject(csr.toASN1Object());
		String sTmp = new String(
				org.bouncycastle.util.encoders.Base64.encode(baos.toByteArray()));
	...
	/**
	 * Installs the Certificate Authority reply returned as part of the signing
	 * request. The certificate being signed will get its certificate chain
	 * updated with the imported certificate(s). An exception will be thrown if
	 * the replied certificate does not match a local certificate or if the
	 * signing authority is not known by the server (i.e. keystore and
	 * truststore files). When <tt>trustCACerts</tt> is set to <tt>true</tt>
	 * then certificates present in the truststore file will be used to verify
	 * the identity of the entity signing the certificate. In case the reply is
	 * composed of more than one certificate then you can also specify if you
	 * want to verify that the root certificate in the chain can be trusted.
	 * 
	 * @param keyStore
	 *            key store where the certificate is stored.
	 * @param trustStore
	 *            key store where ca certificates are stored.
	 * @param keyPassword
	 *            password of the keystore.
	 * @param alias
	 *            the alias of the existing certificate being signed.
	 * @param inputStream
	 *            the stream containing the CA reply.
	 * @param trustCACerts
	 ...
	 * @throws Exception
	 */
	public static boolean installReply(KeyStore keyStore, KeyStore trustStore,
			String keyPassword, String alias, InputStream inputStream,
		...
		if (certs.size() == 1) {
			...
			newCerts = establishCertChain(keyStore, trustStore, null,
					certs.get(0), trustCACerts);
		} else {
			// Reply has a chain of certificates
			newCerts = validateReply(keyStore, trustStore, alias, null, certs,
					trustCACerts, validateRoot);
		}
		if (newCerts != null) {
	 ...
	 * The certificate input stream may contain the signed certificate as well
	 * as its CA chain.
	 * 
	 * @param keyStore
	 *            key store where the certificate will be stored.
	 * @param trustStore
	 *            key store where ca certificates are stored.
	 * @param keyPassword
	 *            password of the keystore.
	 * @param alias
	 *            the alias of the the new signed certificate.
	 * @param pkInputStream
	 *            the stream containing the private key.
	 * @param passPhrase
	 *            is the password phrased used when creating the private key.
	 * @param inputStream
	 *            the stream containing the signed certificate.
	 * @param trustCACerts
	 *            true if certificates present in the truststore file will be
	 *            used to verify the identity of the entity signing the
	 *            certificate.
	 ...
	 * @throws Exception
	 *             if no certificates were found in the inputStream.
	 */
	public static boolean installCert(KeyStore keyStore, KeyStore trustStore,
			String keyPassword, String alias, InputStream pkInputStream,
			final String passPhrase, InputStream inputStream,
			boolean trustCACerts, boolean validateRoot) throws Exception {
		// Check that there is a certificate for the specified alias
		X509Certificate certificate = (X509Certificate) keyStore
				.getCertificate(alias);
		...
		PEMParser pemParser = new PEMParser(
				new InputStreamReader(pkInputStream));
		...
		if (certs.size() == 1) {
			// Reply has only one certificate
			newCerts = establishCertChain(keyStore, trustStore, certificate,
					certs.get(0), trustCACerts);
		} else {
			// Reply has a chain of certificates
			newCerts = validateReply(keyStore, trustStore, alias, certificate,
					certs, trustCACerts, validateRoot);
		}
		if (newCerts != null) {
			keyStore.setKeyEntry(alias, privKey, keyPassword.toCharArray(),
					newCerts.toArray(new X509Certificate[newCerts.size()]));

			// Notify listeners that a new certificate has been created (and
			// signed)
			for (CertificateEventListener listener : listeners) {
				try {
	...
	private static List<X509Certificate> establishCertChain(KeyStore keyStore,
	...
	/**
	 * Builds the certificate chain of the specified certificate based on the
	 * known list of certificates that were issued by their respective
	 * Principals. Returns true if the entire chain of all certificates was
	 * successfully built.
	 * 
	 * @param certificate
	 *            certificate to build its chain.
	 * @param answer
	 *            the certificate chain for the corresponding certificate.
	 * @param knownCerts
	 ...
	 *         built.
	 */
	private static boolean buildChain(X509Certificate certificate,
			LinkedList<X509Certificate> answer,
	...
	/**
	 * Returns a Map where the key holds the certificate issuers and values the
	 * certificates of each issuer.
	 * 
	 * @param ks
	 *            the keystore to get its certs per issuer.
	 * @return a map with the certificates per issuer.
	 * @throws Exception
	 */
	private static Map<Principal, List<X509Certificate>> getCertsByIssuer(
	 ...
	 */
	private static List<X509Certificate> validateReply(KeyStore keyStore,
			KeyStore trustStore, String alias, X509Certificate userCert,
		...
		boolean foundInCAStore = trustCACerts
				&& (trustStore.getCertificateAlias(topCert) != null);
		if (!foundInKeyStore && !foundInCAStore) {
			...
			if (trustCACerts) {
				for (Enumeration<String> aliases = trustStore.aliases(); aliases
					...
					rootCert = (X509Certificate) trustStore
							.getCertificate(name);
					if (rootCert != null) {
						try {
	 ...
	 * 
	 * @param kp
	 *            KeyPair that keeps the public and private keys for the new
	 *            certificate.
	 * @param months
	 *            time to live
	 * @param issuerDN
	 ...
	 * @param signAlgoritm
	 *            Signature algorithm. This can be either a name or an OID.
	 * @return X509 V3 Certificate
	 * @throws GeneralSecurityException
	 * @throws IOException
	 */
	private static synchronized X509Certificate createX509V3Certificate(
			KeyPair kp, int months, String issuerDN, String subjectDN,
			String domain, String signAlgoritm)
			throws GeneralSecurityException, IOException {
		...
		certGenerator.setNotAfter(new Date(System.currentTimeMillis() + months
				* (1000L * 60 * 60 * 24 * 30)));
		...
		certGenerator.addExtension(X509Extensions.SubjectAlternativeName,
				critical, subjectAltNames);

		X509Certificate cert = certGenerator.generateX509Certificate(privKey,
				"BC", new SecureRandom());
	 ...
	 * @return a new public & private key with the specified algorithm (e.g.
	 *         DSA, RSA, etc.).
	 * @throws GeneralSecurityException
	 */
